...
func TestCurrentUpgradeCatalogIncludesAutomaticSupplyChainRelease(t *testing.T) {
	path, available, err := currentUpgradeCatalog().migrationPath("0.10.1", "0.11.0")
	if err != nil {
		t.Fatalf("resolve supply-chain release migration: %v", err)
	}
	if !available || len(path) != 1 {
		t.Fatalf("supply-chain release path = %#v, available = %t", path, available)
	}
	if path[0].ID != "framework-0.10.1-to-0.11.0" || !path[0].Automatic {
		t.Fatalf("supply-chain release migration = %#v", path[0])
	}
}

func TestCurrentUpgradeCatalogPlansSupplyChainReleaseForCustomApplicationProtocol(t *testing.T) {
	root := makeUpgradeProjectRoot(t, currentProjectMetadata("0.10.1", 2, 3))
	var stdout bytes.Buffer
	err := testUpgradeCommand().run(
		[]string{"--plan", "--to", "0.11.0", "--json", "--root", root},
		&stdout,
		&bytes.Buffer{},
	)
	if !errors.Is(err, errUpgradeRequired) {
	...
	if report.Status != upgradeMigrationRequired || !report.PlanAvailable ||
		...
		report.Steps[0].ID != "framework-0.10.1-to-0.11.0" ||
		report.Project.ProtocolVersion != 3 ||
		report.Target.TemplateProtocolVersion != 1 ||
		!hasUpgradeDiagnostic(report, "application_protocol_custom") {
		t.Fatalf("report = %#v", report)
...
func TestCurrentUpgradeCatalogIncludesAutomaticSecureSidecarLaunchRelease(t *testing.T) {
	path, available, err := currentUpgradeCatalog().migrationPath("0.11.0", "0.12.0")
	if err != nil {
		t.Fatalf("resolve secure Sidecar launch migration: %v", err)
	}
	if !available || len(path) != 1 {
		t.Fatalf("secure Sidecar launch path = %#v, available = %t", path, available)
	}
	if path[0].ID != "framework-0.11.0-to-0.12.0" || !path[0].Automatic {
		t.Fatalf("secure Sidecar launch migration = %#v", path[0])
	}
}

func TestCurrentUpgradeCatalogPlansSecureSidecarLaunchForCustomApplicationProtocol(t *testing.T) {
	root := makeUpgradeProjectRoot(t, currentProjectMetadata("0.11.0", 2, 3))
	var stdout bytes.Buffer
	err := testUpgradeCommand().run(
		[]string{"--plan", "--to", "0.12.0", "--json", "--root", root},
		&stdout,
		&bytes.Buffer{},
	)
	if !errors.Is(err, errUpgradeRequired) {
	...
	if report.Status != upgradeMigrationRequired || !report.PlanAvailable ||
		!report.ApplyAvailable || len(report.Steps) != 1 ||
		report.Steps[0].ID != "framework-0.11.0-to-0.12.0" ||
		report.Project.ProtocolVersion != 3 ||
		report.Target.TemplateProtocolVersion != 1 ||
		!hasUpgradeDiagnostic(report, "application_protocol_custom") {
		t.Fatalf("report = %#v", report)
...
func TestCurrentUpgradeCatalogRequiresManualSchemaBaselineAdoption(t *testing.T) {
	path, available, err := currentUpgradeCatalog().migrationPath("0.13.0", "0.14.0")
	if err != nil {
		t.Fatalf("resolve schema baseline migration: %v", err)
	}
	if !available || len(path) != 1 {
		t.Fatalf("schema baseline path = %#v, available = %t", path, available)
	}
	if path[0].ID != "framework-0.13.0-to-0.14.0" || path[0].Automatic {
		t.Fatalf("schema baseline migration = %#v", path[0])
	}
}

func TestCurrentUpgradeCatalogPlansSchemaBaselineForCustomApplicationProtocol(t *testing.T) {
	root := makeUpgradeProjectRoot(t, currentProjectMetadata("0.13.0", 2, 3))
	var stdout bytes.Buffer
	err := testUpgradeCommand().run(
		[]string{"--plan", "--to", "0.14.0", "--json", "--root", root},
		&stdout,
		&bytes.Buffer{},
	)
	if !errors.Is(err, errUpgradeRequired) {
	...
	if report.Status != upgradeMigrationRequired || !report.PlanAvailable ||
		...
		report.Steps[0].ID != "framework-0.13.0-to-0.14.0" ||
		report.Steps[0].Automatic ||
		report.Steps[1].ID != "template-2-to-3" ||
...
func TestCurrentUpgradeCatalogRequiresManualApplicationLifecycleAdoption(t *testing.T) {
	path, available, err := currentUpgradeCatalog().migrationPath("0.14.0", "0.15.0")
	if err != nil {
		t.Fatalf("resolve application lifecycle migration: %v", err)
	}
	if !available || len(path) != 1 {
		t.Fatalf("application lifecycle path = %#v, available = %t", path, available)
	}
	if path[0].ID != "framework-0.14.0-to-0.15.0" || path[0].Automatic {
		t.Fatalf("application lifecycle migration = %#v", path[0])
	}
}

func TestCurrentUpgradeCatalogPlansApplicationLifecycleForCustomProtocol(t *testing.T) {
	root := makeUpgradeProjectRoot(t, currentProjectMetadata("0.14.0", 3, 3))
	var stdout bytes.Buffer
	err := testUpgradeCommand().run(
		[]string{"--plan", "--to", "0.15.0", "--json", "--root", root},
		&stdout,
		&bytes.Buffer{},
	)
	if !errors.Is(err, errUpgradeRequired) {
...
func TestCurrentUpgradeCatalogRequiresManualIOSEmbeddedCoreAdoption(t *testing.T) {
	path, available, err := currentUpgradeCatalog().migrationPath("0.16.0", "0.17.0")
	if err != nil {
		t.Fatalf("resolve iOS Embedded Core migration: %v", err)
	}
	if !available || len(path) != 1 {
		t.Fatalf("iOS Embedded Core path = %#v, available = %t", path, available)
	}
	if path[0].ID != "framework-0.16.0-to-0.17.0" || path[0].Automatic {
		t.Fatalf("iOS Embedded Core migration = %#v", path[0])
	}
}

func TestCurrentUpgradeCatalogPlansIOSEmbeddedCoreForCustomProtocol(t *testing.T) {
	root := makeUpgradeProjectRoot(t, currentProjectMetadata("0.16.0", 6, 3))
	var stdout bytes.Buffer
	err := testUpgradeCommand().run(
		[]string{"--plan", "--to", "0.17.0", "--json", "--root", root},
		&stdout,
		&bytes.Buffer{},
	)
	if !errors.Is(err, errUpgradeRequired) {
	...
	if report.Status != upgradeMigrationRequired || !report.PlanAvailable ||
		report.ApplyAvailable || len(report.Steps) != 2 ||
		report.Steps[0].ID != "framework-0.16.0-to-0.17.0" ||
		report.Steps[0].Automatic ||
		report.Steps[1].ID != "template-6-to-7" ||
		report.Steps[1].Automatic ||
		report.Project.ProtocolVersion != 3 ||
		report.Target.TemplateProtocolVersion != 1 ||
		!hasUpgradeDiagnostic(report, "application_protocol_custom") {
