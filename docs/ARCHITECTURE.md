...
Bridra 0.12 supports Windows, macOS, Linux, Android, iOS, and Web while keeping
...
caching. Both packages remain in one Git repository and use Bridra 0.12.0.
...
- The physical-device gate uses Debug only for automated UI assertions, then a
  ...
  and managed file transfers after restart. A download is interrupted after 32
  KiB; the client must continue with `Range: bytes=32768-` and still verify its
  size, SHA-256, and content. An upload is interrupted at the same offset; the
  client must query the stored offset, continue from it, and let Go consume and
  re-hash the completed file. Its authenticated smoke routes and fault injection
  exist only when the test script explicitly enables `--smoke-stream`,
  `--smoke-download`, `--smoke-download-resume`, and `--smoke-upload-resume`.
  Developer Team and bundle identifier overrides live in ignored
  `Local.xcconfig`.
