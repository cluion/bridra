...
2. Synchronize the framework, CLI, Go module, Dart package, project metadata,
   ...
   ```bash
   make release-prepare VERSION=0.6.1
...
4. Replace `Unreleased` in both changelogs with the intended release date, then
   run `make release-check VERSION=0.6.1 FINAL=1`. The protected release workflow
...
```powershell
.\tool\windows.ps1 -Task release-prepare -Version 0.6.1
.\tool\windows.ps1 -Task release-check -Version 0.6.1
.\tool\windows.ps1 -Task release-check -Version 0.6.1 -Final
```
...
This produces the following under `build/bridra/cli/0.6.1/`:
...
```bash
shasum -a 256 -c build/bridra/cli/0.6.1/SHA256SUMS
bridra version --json
```
...
For Bridra 0.6.1, create the annotated Go submodule tag only after the release
Pull Request is merged and the repository owner gives final authorization:

```bash
git tag -a backend/v0.6.1 -m "Bridra 0.6.1"
git push origin backend/v0.6.1
```
...
```bash
go install github.com/cluion/bridra/backend/cmd/bridra@v0.6.1
bridra version --json
...
upgrade. The `0.6.0` to `0.6.1` path is manual only because projects generated
with `0.6.0` must repair an application-owned `FakeBackend` test that upgrades
never overwrite. Framework APIs, Project Template version `2`, and RPC protocol
