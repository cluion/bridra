...
2. Synchronize the framework, CLI, Go module, Dart package, project metadata,
   ...
   ```bash
   make release-prepare VERSION=0.7.0
...
4. Replace `Unreleased` in both changelogs with the intended release date, then
   run `make release-check VERSION=0.7.0 FINAL=1`. The protected release workflow
...
```powershell
.\tool\windows.ps1 -Task release-prepare -Version 0.7.0
.\tool\windows.ps1 -Task release-check -Version 0.7.0
.\tool\windows.ps1 -Task release-check -Version 0.7.0 -Final
```
...
This produces the following under `build/bridra/cli/0.7.0/`:
...
```bash
shasum -a 256 -c build/bridra/cli/0.7.0/SHA256SUMS
bridra version --json
```
...
For Bridra 0.7.0, create the annotated Go submodule tag only after the release
Pull Request is merged and the repository owner gives final authorization:

```bash
git tag -a backend/v0.7.0 -m "Bridra 0.7.0"
git push origin backend/v0.7.0
```
...
```bash
go install github.com/cluion/bridra/backend/cmd/bridra@v0.7.0
bridra version --json
...
never overwrite. The `0.6.1` to `0.7.0` path is automatic: its new Go Queue and
Scheduler persistence APIs are additive and opt-in. The public Dart API, Project
Template version `2`, project metadata schema `2`, and RPC protocol version `1`
do not change.
