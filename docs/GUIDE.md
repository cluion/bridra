...
Bridra 0.10 is a six-platform framework starter with a typed
...
SemVer (`0.10.1`), the Project Template protocol baseline (`1`), and each
...
go install github.com/cluion/bridra/backend/cmd/bridra@v0.10.1
bridra version --json
bridra upgrade --plan --to 0.10.1 --root /path/to/project
```
...
[RELEASING.md](RELEASING.md). The `0.9.0` to `0.10.1` path contains the
automatic `0.10.0` HTTP-security step and the automatic `0.10.1` diagnostics and
upgrade-planner patch. Existing application-owned server entrypoints are not
overwritten; adopt the production controls deliberately using
...
make release-check VERSION=0.10.1 FINAL=1
```
...
`.\tool\windows.ps1 -Task release-prepare -Version 0.10.1` and the corresponding
...
Each version has its own directory, such as `build/bridra/cli/0.10.1/`, so stale
