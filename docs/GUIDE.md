...
Bridra 0.11 is a six-platform framework starter with a typed
...
SemVer (`0.11.0`), the Project Template protocol baseline (`1`), and each
...
go install github.com/cluion/bridra/backend/cmd/bridra@v0.11.0
bridra version --json
bridra upgrade --plan --to 0.11.0 --root /path/to/project
```
...
[RELEASING.md](RELEASING.md). The `0.9.0` to `0.11.0` path contains the
automatic `0.10.0` HTTP-security step, the `0.10.1` diagnostics and
upgrade-planner patch, and the runtime-neutral `0.11.0` supply-chain release.
...
make release-check VERSION=0.11.0 FINAL=1
```
...
`.\tool\windows.ps1 -Task release-prepare -Version 0.11.0` and the corresponding
...
Each version has its own directory, such as `build/bridra/cli/0.11.0/`, so stale
