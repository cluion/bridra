...
SemVer (`0.17.0`), the Project Template protocol baseline (`1`), and each
...
go install github.com/cluion/bridra/backend/cmd/bridra@v0.17.0
bridra version --json
bridra upgrade --plan --to 0.17.0 --root /path/to/project
```
...
[RELEASING.md](RELEASING.md). The `0.9.0` to `0.17.0` path contains the
...
`0.15.0` Application lifecycle, `0.16.0` macOS resource handoff, and `0.17.0`
iOS Embedded Core steps are
manual because Bridra does not overwrite application-owned schema baselines,
app wiring, Sidecar／Embedded Core entrypoints, or native runners.
...
make release-check VERSION=0.17.0 FINAL=1
```
...
`.\tool\windows.ps1 -Task release-prepare -Version 0.17.0` and the corresponding
...
Each version has its own directory, such as `build/bridra/cli/0.17.0/`, so stale
