...
Bridra 0.6 is a six-platform framework starter with a typed
...
SemVer (`0.6.1`) and wire protocol version (`1`) evolve independently.
...
go install github.com/cluion/bridra/backend/cmd/bridra@v0.6.1
bridra version --json
bridra upgrade --plan --to 0.6.1 --root /path/to/project
```
...
[RELEASING.md](RELEASING.md). The `0.5.0` to `0.6.1` transition is automatic
...
make release-check VERSION=0.6.1 FINAL=1
```
...
`.\tool\windows.ps1 -Task release-prepare -Version 0.6.1` and the corresponding
...
Each version has its own directory, such as `build/bridra/cli/0.6.1/`, so stale
