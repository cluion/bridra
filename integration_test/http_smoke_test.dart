...
const smokeDownloadMethod = 'bridra.smoke.download';
const smokeDownloadName = 'bridra-smoke.bin';
const smokeDownloadMediaType = 'application/octet-stream';
const smokeDownloadBlock = 'bridra-managed-download-smoke|';
...
Future<void> _verifyManagedDownload() async {
  final client = HttpRpcClient(
    endpoint: Uri.parse(smokeBackendUrl),
    token: smokeBackendToken,
  );
  try {
    final reply = await client.call(smokeDownloadMethod);
    ...
    final expected = utf8.encode(
      List.filled(smokeDownloadBlockCount, smokeDownloadBlock).join(),
    );
    expect(reference.name, smokeDownloadName);
    expect(reference.mediaType, smokeDownloadMediaType);
    expect(reference.size, expected.length);
    expect(reference.sha256, smokeDownloadSha256);
    expect(reference.localPath, isNull);

    final downloaded = <int>[];
    await for (final chunk in client.download(reference)) {
